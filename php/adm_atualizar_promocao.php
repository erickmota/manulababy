...
$id = str_replace(array(";", "'", "--", "/", "*", "xp_", "XP_", "SELECT" , "INSERT" , "UPDATE" , "DELETE" , "DROP", "select" , "insert" , "update" , "delete" , "drop"), "", htmlentities($_POST["id"]));

/* echo "{$nome}<br>{$quantidade}<br>{$preco}<br>{$descricao}<br>"; */

$funcAtualizarPromocao = $classeProdutos->atualizar_promocao($nome, $quantidade, $preco, $descricao, $id);

if($funcAtualizarPromocao == false){
